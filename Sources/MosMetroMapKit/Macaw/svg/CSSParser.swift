...
class CSSParser {
    ...
    func parseSelector(text: String) -> MSelector {
        if text.first == "#" {
    ...
    fileprivate func getStyles(selector: MSelector) -> [String: String]? {
        switch selector {
        case .byId(let id):
    ...
    fileprivate func setStyles(selector: MSelector, styles: [String: String]) {
        switch selector {
        case .byId(let id):
