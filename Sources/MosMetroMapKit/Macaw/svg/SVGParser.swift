...
open class SVGParser {
    ...
    fileprivate func parseLine(_ line: XMLIndexer) -> MLine? {
        guard let element = line.element else {
            return .none
        }

        return MLine(x1: getDoubleValue(element, attribute: "x1") ?? 0,
    ...
    fileprivate func parseStops(_ stops: [XMLIndexer], groupStyle: [String: String] = [:]) -> [MStop] {
        var result = [MStop]()
    ...
    fileprivate func parseStop(_ stop: XMLIndexer, groupStyle: [String: String] = [:]) -> MStop? {
        ...
        return MStop(offset: offset, color: color)
    }
