...
open class LinearGradient: Gradient {
    ...
    public init(x1: Double = 0, y1: Double = 0, x2: Double = 0, y2: Double = 0, userSpace: Bool = false, stops: [MStop] = []) {
        self.x1 = x1
        self.y1 = y1
        self.x2 = x2
        self.y2 = y2
    ...
    public convenience init(degree: Double = 0, from: Color, to: Color) {
        self.init(degree: degree, stops: [MStop(offset: 0, color: from), MStop(offset: 1, color: to)])
    }

    public init(degree: Double = 0, stops: [MStop]) {
