...
public enum VAAnimationType: Animation {

    case from(direction: VADirection, offset: CGFloat)
    ...
    public static func random() -> Animation {
        ...
        if index == 1 {
            return VAAnimationType.vector(CGVector(dx: .random(in: -10...10), dy: .random(in: -30...30)))
        } else if index == 2 {
            let scale = Double.random(in: 0...ViewAnimatorConfig.maxZoomScale)
            return VAAnimationType.zoom(scale: CGFloat(scale))
        }
        let angle = CGFloat.random(in: -ViewAnimatorConfig.maxRotationAngle...ViewAnimatorConfig.maxRotationAngle)
        return VAAnimationType.rotate(angle: angle)
    }
}
